...
//apliquem social laws no es pot moure si hi ha cotxe a on es vol moure i preferència a les caselles inicials
...
{				
        ...
        for(i=8;i<12;i++)
	{
		...
		{
			branch[i]=1;
			if(v[nextp[i][0]]==0 && v[mlook[i]]==0)		//es pot moure a la casella central
                	        branch[i]+=1;
			if(v[nextp[i][1]]==0)		//es pot moure a la casella final
                	        branch[i]+=1;
		}
	}
        for(i=1;i<8;i+=2)
	...
	//massa variables div, s'ha de revisar i potser fer-la més senzilla
{
	...
	//llavors a cada segment de les divisions anem fent el que correspon, moure cotxes (afecta a la casella a on es mou) entrar cotxes o no fer res
        ...
        for(i=8;i<12;i++)			//possicions centrals
        {
                div1=div2/branch[i];
                div2=div1;
                if(branch[i]==3)		//hi ha cotxe i es pot moure a les dues caselles
                {
                        div3=div1;
                        do
                        {

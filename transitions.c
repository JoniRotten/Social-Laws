...
int compare(int *, int *);  
int treenext(int *,int *);
void tree(int *,int *, int **, int);
...
void printfile(int, int, int, struct position *, struct transition *, struct collision *, FILE *, FILE *, FILE *, FILE *,FILE *);
...
void main(void)
{
	...
	struct collision *vcol;					//vector de colisions
        ...
        time_t timer;
        pf=fopen("new_positions.dat","w+");
        qf=fopen("new_crash.dat","w+");
...
//        printf("\n\n\nThe current time is %s.\n\n\n",asctime(localtime(&timer)));
	...
	fillcol(vtran,vcol,ntran);
        timer=time(NULL);
        printf("\n\n\nThe current time is %s.\n\n\n",asctime(localtime(&timer)));
        printfile(nnode,ntran,ncol,node,vtran,vcol,pf,qf,rf,sf,tf);
        timer=time(NULL);
        printf("\n\n\nThe current time is %s.\n\n\n",asctime(localtime(&timer)));
...
int treenext(int *branch, int *v)
	//calcula el nombre de possicions next per un vector, es podria integrar amb tree, pero em lio amb els punters
...
{				
        ...
        for(i=8;i<12;i++)
                if(v[i]==0)
                        ...
                        branch[i]=3;                 //el coche es queda quiet, es mou recte al centre, o es mou a la dreta a caselles finals
        for(i=1;i<8;i+=2)
                if(v[i]==0)
                        branch[i]=1;                 //no passa res
                else
...
void tree(int *branch, int *v, int **m, int cont)   
	//crea totes les possicions next per un vector a una matriu m
	//rep com a paràmetres el vector de possició, el vector branch de treenext,la matriu m i el nombre de vectors next calculat a treenext
	...
	//rep com a parámetres els vectors vuits de nodes i punters a nodes crash i nocrash, aquí els omple
{
	...
	//cont es el nombre de vectors next retornat per treenext
	...
	//matriu de nodes next, es crea i es desassigna dins del bucle principal
        branch=vector(12);
	//branch es el vector de possibilitats calculat a treenext 
        jprev=vector(N); 
		...
		//si no es crash, l'afegim a nocrashv, el marquem com nocrash i calculem la matriu m de vectos next
                {
                        ...
                        node[q].iscrash=0;
                        nocrashn+=1;
                        cont=treenext(branch,node[q].pos);
                        m=matrix(cont,12);
                        tree(branch,node[q].pos,m,cont);
...
void movetran(int * branch, int * move, int next, int nnext)   //es poden arribar a moure 13 caselles, si entra un de fora
{
	...
	int * div;
	div=vector(12);			//el vector div s'hauria de passar com a parámetre i calcular-se només una vegada a treenext
	for(i=0;i<16;i++)
		move[i]=i;
	div[0]=branch[0];
	segment=nnext/div[0];
...
void printfile(int nnode, int ntran, int ncol, struct position * node, struct transition * vtran, struct collision * vcol, FILE *pf, FILE *qf, FILE *rf, FILE *sf, FILE *tf)
